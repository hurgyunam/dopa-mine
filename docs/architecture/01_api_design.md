...
플로우 상세는 별도 문서에서 관리한다.

| 문서 | 내용 |
| --- | --- |
| `docs/architecture/02_auth_flow.md` | 앱 시작/로그인/토큰 갱신/로그아웃 시퀀스 |
| `docs/architecture/03_session_save_flow.md` | 세션 생성·세트 저장·멱등·재시도·완료 확정 플로우 |
...
| 날짜 | 변경 요약 | 작성자 |
| --- | --- | --- |
| 2026-02-27 | 인증 플로우·세션 저장 플로우를 별도 문서(`02_auth_flow`, `03_session_save_flow`)로 분리 | @cursor-agent |
